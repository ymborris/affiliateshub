...
rate. Both lists draw it, in **both** the card and the table view, in a larger gold
size so it is readable at arm's length on a phone.
...
| Path | What it is |
|---|---|
| `index.html` | The dashboard. Same UI as before, but the inline dataset is gone and it loads `data/streaks.json`. Both list tabs (Streaks and Consistency) offer Cards and Table, each remembering its own choice. |
...
**The site shows a blue "Interactive preview" banner and the data never changes.**
The preview file (`hotstreaks-fix-preview.html`, ~3.7 MB) has been uploaded as `index.html`.
It embeds a frozen snapshot and deliberately thins the football list, and it is for looking
at the design only. Upload the real `index.html` instead (~65 KB, no banner) — that one
fetches `data/streaks.json`, so the nightly workflow keeps it current. Sizes tell them
apart at a glance: 65 KB = right file, 3.7 MB = preview.

**The page freezes when sorting (fixed 19 Sep 2026).** "Most runs" rebuilt its team-run
table inside the sort comparator, so every comparison recomputed it — on the full football
list that blocked the browser for over a minute, and every tap afterwards queued behind it
until the page was refreshed. Sorts now read each key once (decorate-sort-undecorate) and
the count table is cached per dataset. If you ever add a sort, never call anything that
walks the list from inside the comparator.
